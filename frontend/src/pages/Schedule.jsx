...
const Schedule = () => {
  ...
  const {
    ...
    queryKey: ["schedule"],
    queryFn: () => api.get("/schedule/get-all"),
    staleTime: Infinity,
  ...
  const handleCancelEdit = (dayIndex) => {
    // Discard changes by invalidating to trigger refetch which resets state via useEffect
  ...
  const handleImportJson = () => {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        if (importedCount === 0)
          throw new Error("No valid day keys (0-6) found");

        setSchedule(newSchedule);
        setOffDays(newOffDays);
        setIsEditing(newEditing);
        toast.success(
          "Multi-day schedule imported! Please review and finalize each day.",
        );
      } else {
      ...
      toast.error(
        err.message || "Invalid JSON format. Please check the structure.",
      );
    }
  };
  ...
  const handleSaveDay = async (dayIndex) => {
    ...
    try {
      ...
      // Invalidate to update server state representation
      queryClient.invalidateQueries(["schedule"]);
      setIsEditing((prev) => ({ ...prev, [dayIndex]: false }));
  ...
  const isCurrentEditing =
    isEditing[selectedDay] || (currentEntries.length === 0 && !isCurrentOffDay);

  if (isLoading && !schedule[0]) {
    return (
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          <Tooltip title="Refresh">
            <IconButton
              onClick={() =>
                refreshSync().then(() => toast.success("Schedule updated"))
              }
              sx={{
              ...
              >
                <Stack direction="row" spacing={2.5} alignItems="center">
                  ...
                  <Box>
                    ...
                    >
                      {isCurrentOffDay
                        ? "No classes - recharging"
                        : `${currentEntries.length} active sessions`}
                    </Typography>
                  </Box>
                </Stack>
                <Switch
                    ...
                    {isCurrentEditing ? (
                      <Box sx={{ p: 3 }}>
                        <Grid container spacing={2.5}>
                          ...
                          <IconButton
                            onClick={() =>
                              handleRemoveEntry(selectedDay, entry.id)
                            }
                            sx={{
                              position: "absolute",
                              top: 12,
          ...
          {isCurrentOffDay && (
            ...
            >
              ...
              >
                ...
                >
                  No academic sessions on the horizon. Time to power down and
                  recharge.
                </Typography>
              </Box>
            </motion.div>
